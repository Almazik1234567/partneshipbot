...
module.exports = async (client, message) => {
  ...
  if (!message.guild && cmd.conf.guildOnly) return client.embed.send(message, { desc: 'Эта команда недоступна через личные сообщения. Пожалуйста, запустите эту команду в гильдии.' })

  // Is the bot currently under going maintenence?
  if (client.config.botMaintenance && message.author.id !== client.config.ownerID) return client.embed.send(message, { desc: 'В настоящее время ботом может управлять только владелец бота. Извините за причиненные неудобства.' })
